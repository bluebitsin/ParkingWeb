...
public class UserServiceImpl implements UserService {
	...
	public User getLoginAgent(String mobile, String password) {
		...
		if (mobile != null && !mobile.trim().isEmpty() && password != null && !password.trim().isEmpty()) {
			...
			user = getAgentDetails(mobile, password);
			return user;
	...
	private User getAgentDetails(String mobile, String password) {
		...
		try {
			...
			String hqlQuery = "FROM Agent WHERE mobile = :mobile AND agentPassword = :password";
			Query query = session.createQuery(hqlQuery, Agent.class);
			query.setParameter("mobile", Integer.parseInt(mobile));
			query.setParameter("password", password);
			...
			user.setVechileModel("");
			user.setVechileNumber("");
			user.setAgentCode(agent.getAgentCode());
